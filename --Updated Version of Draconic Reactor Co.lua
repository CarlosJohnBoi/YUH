...
inputflow_gate = f.periphSearch("flow_gate")
flow_gate = peripheral.wrap(flow_gateSide)
...
if flow_gate == null then
	error("No valid flow_gate was found")
end
...
function buttons()
  ...
  while true do
    ...
    if yPos == 8 then
      local cFlow = flow_gate.getSignalLowFlow()
      ...
      flow_gate.setSignalLowFlow(cFlow)
    end
    ...
    if yPos == 10 and autoInputGate == 0 and xPos ~= 14 and xPos ~= 15 then
      ...
      inputflow_gate.setSignalLowFlow(curInputGate)
      save_config()
    end
...
function update()
  while true do 
    ...
    print("Output Gate: ", flow_gate.getSignalLowFlow())
    print("Input Gate: ", inputflow_gate.getSignalLowFlow())
    ...
    f.draw_text_lr(mon, 2, 7, 1, "Output Gate", f.format_int(flow_gate.getSignalLowFlow()) .. " rf/t", colors.white, colors.blue, colors.black)

    -- buttons
    drawButtons(8)

    f.draw_text_lr(mon, 2, 9, 1, "Input Gate", f.format_int(inputflow_gate.getSignalLowFlow()) .. " rf/t", colors.white, colors.blue, colors.black)
    ...
    if ri.status == "charging" then
      inputflow_gate.setSignalLowFlow(900000)
    ...
    if ri.status == "online" then
      if autoInputGate == 1 then 
        ...
        inputflow_gate.setSignalLowFlow(fluxval)
      else
        inputflow_gate.setSignalLowFlow(curInputGate)
      end
    end
